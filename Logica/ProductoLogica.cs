        ...
        public int Existe(string codigo, int defaultid, out string mensaje)
        {
            ...
            using (SQLiteConnection conexion = new SQLiteConnection(Conexion.cadena))
            {
                try
                {
                    ...
                    query.AppendLine("SELECT count(*)[resultado] FROM PRODUCTO_FARMACIA WHERE upper(Codigo) = upper(@pcodigo) AND IdProducto != @defaultid");
        ...
        public int Eliminar(int id)
        {
            ...
            try
            {
                using (SQLiteConnection conexion = new SQLiteConnection(Conexion.cadena))
                {
                    conexion.Open();
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("DELETE FROM PRODUCTO_FARMACIA WHERE IdProducto = @id;");
                    SQLiteCommand cmd = new SQLiteCommand(query.ToString(), conexion);
